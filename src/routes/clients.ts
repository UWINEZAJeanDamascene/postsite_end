...
router.get('/:id', authenticateToken, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const client = await Client.findOne({ _id: id, company_id }).lean()
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const existing = await Client.findOne({ name: data.name, company_id })
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await ActionLogService.logFromRequest(
        ...
        ActionType.UPDATE,
        ResourceType.CLIENT,
        `${isActive ? 'Activated' : 'Deactivated'} client ${client.name}`,
        { resourceId: client._id.toString(), resourceName: client.name }
      )
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const client = await Client.findOneAndDelete({ _id: id, company_id })
